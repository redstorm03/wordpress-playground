...
class Ufa
{
	...
	public function Deposit($username, $amount, $phone, $name,$admin,$becuz,$bankin)
	{	
		$iddp = $_POST["id"];
		$idcb = $_POST["id"];
		@$aspx_auth = $this->get_created_cookie_login('cookie_login');
	...
	public function GetBalance($username)
	{
		...
		if ($check['stats'] == true) {
			...
			//print_r($cellData[9]);
			$json =  json_encode(['Balance' => $cellData[10]], JSON_UNESCAPED_UNICODE);
	...
	public function GetBalanceGame($username)
	{
		@$aspx_auth = $this->get_created_cookie_login('cookie_login');
		$ASPXAUTH = @$aspx_auth['aspx'];
		$check = $this->check_login($ASPXAUTH);
		...
		if ($check['stats'] == true) {
			...
			//print_r($cellData[9]);
			$json =  json_encode(['Balance' => $cellData[7]], JSON_UNESCAPED_UNICODE);
			return $json =  str_replace(array('\t', '\r', '\n', '            '), '', $json);
		} else {
			$cookie = $this->genarate_cookie('cookie_capt');
			$cookie1 = $this->genarate_cookie('cookie_login');
	...
	public function Withdraw($username, $amount, $phone, $name, $admin, $becuz)
	{
		$idwd = $_POST["id"];
		@$aspx_auth = $this->get_created_cookie_login('cookie_login');
		$ASPXAUTH = @$aspx_auth['aspx'];
...
$data4 = $api->GetBalanceGame($agent.$username_mb);
$Balance2=json_decode($data4);
...
if(isset($_GET['deposit'])) {
			...
			if ($admin=='') {
	 		...
	 		$name=$_POST['name'];
	 		$becuz=$_POST['becuz'];
	 		$bankin=$_POST['bankin_dp'];
	 		if ($username!='') {
	 			
	 			$data22 = $api->GetBalance($username);
				$Balance21=json_decode($data22);
				$Balance222=$Balance21->Balance;

				$data7 = $api->GetBalanceGame($username);
                ...
                WHERE phone_dp='$phone' ORDER BY id DESC LIMIT 1";
                $result9 = mysqli_query($con, $sql) or die ("Error in query: $sql " . mysqli_error());
	 		}
	 		if ($result9 == TRUE) {
	 			echo $api->Deposit($username,$amount,$phone,$name,$admin,$becuz,$bankin);
	 		}
...
if(isset($_GET['withdraw'])) {
	 		...
	 		$admin=$_POST['admin'];
	 		$becuz=$_POST['becuz'];
	 	echo $api->Withdraw($username,$amount,$phone,$name,$admin,$becuz);

	 	}


//echo $api->agent_info(); //ข้อมูลเอเย่น
//echo $api->turnover('ufmgk566Aio4c'); // ดูยอดเทิร์นสมาชิก
